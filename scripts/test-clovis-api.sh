...
CLOVIS_API_KEY="sv-prod-a9f2k!d7z3lq:8mrtu0nvb,gx4wclivusy..!"
CLOVIS_API_BASE="http://localhost:5174"
...
curl -X GET "${CLOVIS_API_BASE}/api/stratus-product-meta" \
...
curl -X GET "${CLOVIS_API_BASE}/api/stratus-product-events" \
  -H "Origin: https://stratus-ventures.org" \
  -H "clovis-api-key: ${CLOVIS_API_KEY}" \
  -v
...
curl -X OPTIONS "${CLOVIS_API_BASE}/api/stratus-product-meta" \
  -H "Origin: https://stratus-ventures.org" \
