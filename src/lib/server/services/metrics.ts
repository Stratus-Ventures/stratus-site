...
export async function storeProductMetrics(
	...
	try {
		...
		await database.delete(stratusMetrics).where(eq(stratusMetrics.product_name, productName));
		...
		logger.info(
			`Successfully stored ${storedCount} metrics for product ${productName} (replaced existing)`
		);
	} catch (error) {
 ...
 */
export async function syncProductMetrics(
	product: StratusProduct,
	database = db
): Promise<{
	success: boolean;
	metricsCount?: number;
