...
>
	...
	>
		{#each products as product}
			...
			<ProductItem {product} {canEdit} bind:state={productState} onEdit={handleEditProduct} />
		{/each}
		...
		{#if products.length === 0}
			...
			>
				{#if error}
					<h3 class="title text-center text-secondary-fg">{error}</h3>
				{:else}
					<h3 class="title text-center text-secondary-fg">No Products Yet</h3>
				{/if}
			</div>
		{/if}
	</div>
